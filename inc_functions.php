...
function zeroDate($p_date, $show = 1){
	if (is_null($p_date)){
		return '--';
	}
	if (strlen($p_date) == 0){
...
function isEmail($p_value, $p_chkdns = false){
	$res = preg_match('/^[^@]+@[a-zA-Z0-9._-]+\.[a-zA-Z]+$/', $p_value);
	if (!$p_chkdns){
		return $res;
	}
	if (!$res) return false;
...
function getOptions($p_table, $p_label, $p_value, $p_selected, $p_default = '', $p_order = ''){
...
function getYearOptions($val, $start = 2013, $ext = 2){
	$sTemp = '';
...
function getUpdateQuery($p_table, $p_idField, $p_fields = '', $p_types = ''){
	$sql = "DESCRIBE $p_table";
	$res = mysqli_query($GLOBALS['cn'], $sql);
	if (!$res){
		return '';
...
function getInsertQuery($p_table, $p_idField, $p_fields = '', $p_types = ''){
	$sql = "DESCRIBE $p_table";
	$res = mysqli_query($GLOBALS['cn'], $sql);
	if (!$res){
		return '';
...
function numberToMonth($val){
	$labels = array('Any', 'January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'Novermber', 'December');
	return $labels[$val];
}
